    ...
    C: Has<XSpanIdString> + Has<Option<Authorization>> + Send + Sync + 'static
{
        ...
        C: Has<XSpanIdString> + Has<Option<Authorization>> + Send + Sync + 'static
    {
        ...
        match method {
            ...
            hyper::Method::POST if path.matched(paths::ID_USER_RESTORE_PASSWORD_CHANGE_PASSWORD) => {
                {
                    let authorization = match *(&context as &dyn Has<Option<Authorization>>).get() {
                        Some(ref authorization) => authorization,
                        None => return Ok(Response::builder()
            ...
            _ if path.matched(paths::ID_USER_RESTORE_PASSWORD_CHANGE_PASSWORD) => method_not_allowed(),
            _ if path.matched(paths::ID_USER_RESTORE_PASSWORD_TOKEN_REQUEST) => method_not_allowed(),
...
impl<T> RequestParser<T> for ApiRequestParser {
    fn parse_operation_id(request: &Request<T>) -> Option<&'static str> {
        ...
        match *request.method() {
            // AnonymousRegisters - POST /user/anonymous-register
            hyper::Method::POST if path.matched(paths::ID_USER_ANONYMOUS_REGISTER) => Some("AnonymousRegisters"),
            // AnonymousRequestsRestoreToken - POST /user/restore-password/token-request
            hyper::Method::POST if path.matched(paths::ID_USER_RESTORE_PASSWORD_TOKEN_REQUEST) => Some("AnonymousRequestsRestoreToken"),
            // AnonymousWithTokenChangesPassword - POST /user/restore-password/change-password
            hyper::Method::POST if path.matched(paths::ID_USER_RESTORE_PASSWORD_CHANGE_PASSWORD) => Some("AnonymousWithTokenChangesPassword"),
            // UserLoginsWithPassword - POST /user/password-login
            hyper::Method::POST if path.matched(paths::ID_USER_PASSWORD_LOGIN) => Some("UserLoginsWithPassword"),
            // UserRegistersWithPassword - POST /user/register
            hyper::Method::POST if path.matched(paths::ID_USER_REGISTER) => Some("UserRegistersWithPassword"),
            _ => None,
        }
    }
}
